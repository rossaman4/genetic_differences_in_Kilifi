	  ...
	  newInf[length(newInf[,1]), "pID"]<-sample(numPple,1)
        ...
        if (rand3<probRecombination){  
          ...
          if (is.data.frame(potRecombInf)) {
            ...
            idR <- sample(idR, 1,replace=FALSE)  
            rndmgenotype<-potRecombInf[idR,]
            
            for (h in 1:numChrPerInf){
		newInf[length(newInf[,1]),c((numPreSNPcol+startChrcol[h]):(numPreSNPcol+endChrcol[h]))]<-getRecombinant(newInf[length(newInf[,1]), 
...
getPotentialImportedInfections<-function() {
  ...
  potAddt <- sample(1:2850, nImp, replace=TRUE) #time changed t reflect new timesteps in obsInf
...
setUpInitialCurrentGenotypes<-function() {
  ...
  colnames(current_genotypes)[1:7]<-c("location.id","pID","clear","time","numCommInf", "locx", "locy")
...
# label is genoID
recentInfections<-cbind(current_genotypes[,c(1,2,4,6,7)],genoID)
colnames(recentInfections)<-c("location.id","pID", "time", "locx", "locy", "genoID")
...
names(Hospdata)[62]<-"dist"
names(Hospdata)[63]<-"set"
names(Hospdata)[64]<-"setInf"
